...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect,QueenAnt) and insect==insect.check_list[0]:
                self.ant=insect
            # END Problem 13
    ...
    def nearest_bee(self, hive):
        ...
        while count<self.min_range:
            ...
            if temp_place==hive:
                return None
            count+=1
        while not random_or_none(temp_place.bees) and temp_place.entrance is not hive and count<self.max_range:
            temp_place=temp_place.entrance
            count+=1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0 and temp_place.bees:
            for i in range(len(temp_place.bees)-1,-1,-1):
                temp_place.bees[i].reduce_armor(self.damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            if random_or_none(temp_place.bees):
                self.eat_bee(random_or_none(temp_place.bees))
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self != self.check_list[0]:
            amt=self.armor
            self.reduce_armor(amt)
        else:
            temp_place=self.place
            while temp_place.exit:
                if temp_place.exit.ant:
                    if temp_place.exit.ant.container and temp_place.exit.ant.ant:
                        if temp_place.exit.ant not in self.temp_list:
                            temp_place.exit.ant.damage *= 2
                            self.temp_list.append(temp_place.exit.ant)
                        if temp_place.exit.ant.ant not in self.temp_list:
                            temp_place.exit.ant.ant.damage *= 2
                            self.temp_list.append(temp_place.exit.ant.ant)
                    if temp_place.exit.ant not in self.temp_list:
                        temp_place.exit.ant.damage*=2
                        self.temp_list.append(temp_place.exit.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0 and self == self.check_list[0]:
            bees_win()
        # END Problem 13
